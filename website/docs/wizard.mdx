...
**What you do:** Read-only review of the actual config files the wizard will generate during deployment — `.env`, `appConfig.json`, `production.js`, `init.sql`, the Docker Compose files, and the Envoy proxy config.
...
- **File Sharing (NFS / SMB)** — create up to 3 containerized file servers that mount host paths into the platform. NFS isn't available on Windows hosts; use SMB there.
...
:::caution File-sharing paths are not auto-created
The wizard format-validates the host paths but doesn't check they exist. Make sure the directories are created on the host before deploying, or the mounts will fail at container start.
:::
