...
:::danger This permanently deletes all your Olympus data
Uploaded files, user accounts, chat history and embeddings — there is no undo. Docker **images** are kept, so the reinstall is fast.
...
:::caution Deleting the folder alone is not enough
The data lives in Docker **volumes**, not in `~/olympus`. Removing only the folder leaves all 11 volumes behind — and because the Compose project keeps the same name, the next install **reuses the old Postgres volume**, skips `init.sql`, and comes up with no admin account. Always run the volume sweep above.
:::
